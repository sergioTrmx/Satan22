...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:3.0\n' 
            + 'FN:♞☞  S̵̙̥̬̥̖̲͈̺̫̾̈́͗̇͝e̶̠̞̦̬̤̲͙̊̒̐̓̽̚̚͝ŗ̵̧̢͍̪̦͕̦̞͒̊̍̿̀̑g̷͕͉͖̺̯͐͐͆i̴̧̯̲̩͙̺̫̟̕͜o̶̢͖͇͉͚̿̑͛̑͜ͅ  🐲🏆 Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
...
const config = {
    XBOT: '❉𝕾𝖊𝖗𝖌𝖎𝖔-𝕭𝖔𝖙❉', 
    instagram: 'https://www.instagram.com/p/CL_ViouF8IU/?igshid=1vk8patcw7tf9', 
    nomer: 'wa.me/+525623741080',
    whatsapp: 'https://chat.whatsapp.com/HXr3jVDApfQEeoA71ACnNa', 
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@pana_sergio01`)
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Que bueno que te fuiste @${num.split('@')[0]} Bye aborto`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '️Terminado 🖤',
				error: {
					stick: 'Ha fallado, intenta luego',
					Iv: 'Link invalido'
				},
				only: {
					group: '❬❗❭ Solo grupo ',
					ownerG: '❬❗❭ Solo dueño del bot ',
					ownerB: '❬❗❭  Solo dueño del bot ',
					admin: '❬❗❭ Solo admin del grupo ',
					Badmin: '❬❗❭ Debes ser admin del grupo '
				}
			}
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('525623741080@s.whatsapp.net', options, text, {quoted: mek})
                    reply('El problema ha sido reportado al propietario del bot, informe falso / main2 no será respondido.')
                    break
                case 'ssweb':
				...
				case 'chord':
					...
					break
                case 'lirik':
                    if (args.length < 1) return reply('el título de la canción donde')
                    teha = body.slice(7)
                ...
                case 'anjing':
					...
					break
                case 'spamcall':
                   if (args.length < 1) return ('Texto para el llamado')
                   weha = body.slice(10)
                ...
                case 'indohot':
                   ...
                   break
				case 'ytmp4':
					if (args.length < 1) return reply('Ingresa URL del video')
            ...
            break	
				...
				case 'tts':
					...
					if (args.length < 2) return client.sendMessage(from, 'Ingresa el texto', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('Ha habido un fallo, intenta luego')
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				await client.leaveGroup(from, 'Adios perros', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' *Texto para el llamado* ?') 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ MENSAJE DE DIFUSIÓN ❯\n\n${body.slice(4)}`})
						}
						reply('Mensaje difundido ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ MENSAJE DE DIFUSIÓN ❯\n\n${body.slice(4)}`)
						}
						reply('Mensaje difundido ')
					}
					break
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Foto del grupo cambiada con exito')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Aquien quieres añadir?')
					if (args[0].startsWith('08')) return reply('Usar el código del pais')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Error al añadir, quiza tenga la privacidad activa')
					}
					break
					case 'grup':
					case 'group':
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "-·=»‡«=·- 𝕤𝔸Ｔ𝐚Ⓝ -·=»‡«=·-", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Este es el numero del admin, nada de spam pls',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Nombre del grupo cambiado con exito', text, {quoted: mek})
                break
                case 'setdesc':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Descripcion del grupo cambiada con exito', text, {quoted: mek})
                break
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Ha sido desacendido🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Oye @${mentioned[0].split('@')[0]} Has sido desacendido🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Has sido ascendido (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Felicidades @${mentioned[0].split('@')[0]} Has sido ascendido (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Expulsado por castroso🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Expulsado por castroso @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('Tagea un sticker ')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Ha fallado, intenta luego')
				...
				case 'nsfw':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Sucio')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('Activo')
						nsfw.push(from)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('NSFW activado con exito')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('NSFW desactivado con exito')
					} else {
						reply(' *Para activar pon 1 y desactivar 0* \nEjemplo: 𝗻𝘀𝗳𝘄 𝟭')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Abierto !!!')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬La bienvenida ha sido activada con exito')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('La bienvenida ha sido desactivada')
					} else {
						reply(' *Para activar pon 1 y desactivar 0* \n *Ejemplo: ${prefix}welcome 1*')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *TÚ QUE* ?') 
					if (args.length < 1) return reply(' *Tagea a quien clonaras* ')
					...
					try {
						...
						mentions(`Foto de perfil de @${id.split('@')[0]} Ha sido clonada`, [jid], true)
					} catch (e) {
						reply(' Ha fallado, intenta luego* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(' *Enviar foto con movimiento* ')
					}
					break
				default:
